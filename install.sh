...
clear
    ...
    echo -e "             ⊂(___)づ  ⋅˚₊‧ ଳ ‧₊˚ ⋅"              
    echo -e "\n ${g}[${n}${KER}${g}] ${c}Exiting ${g}FmCodex Banner \033[1;36m"
    echo
    cd "$HOME"
    rm -rf FmCodex
    exit 0
...
echo
echo
type_effect "[ FmCodex STARTED]" 0.04
sleep 0.2
type_effect "「HELLO DEAR USER I•M X-Fm√ 」" 0.08
sleep 0.5
type_effect "【FmCodex WILL PROTECT YOU】" 0.08
sleep 0.7
type_effect "<GOODBYE>" 0.08
sleep 0.2
type_effect "[ENJOY OUR FmCodex]" 0.08
sleep 0.5
type_effect "!...............¡" 0.08
echo
sleep 2
...
echo
    ...
    if [ ! -f "$HOME/.FmCodex-X-Fm√/report" ]; then
        mv $HOME/FmCodex/files/report $HOME/.FmCodex-X-Fm√ &
        show_spinner "FmCodex-report"
    fi
    if [ ! -f "/data/data/com.termux/files/usr/bin/chat" ]; then
        mv $HOME/FmCodex/files/chat.sh /data/data/com.termux/files/usr/bin/chat &
...
ds="$HOME/.termux"
dx="$ds/font.ttf"
X-Fm√="$ds/colors.properties"
if [ -f "$dx" ]; then
    echo
else
	cp $HOME/FmCodex/files/font.ttf "$ds"
fi
...
if [ -f "$X-Fm√" ]; then
	...
	cp $HOME/FmCodex/files/colors.properties "$ds"
fi
cp $HOME/FmCodex/files/ASCII-Shadow.flf $PREFIX/share/figlet/
mv $HOME/FmCodex/files/remove /data/data/com.termux/files/usr/bin/
chmod +x /data/data/com.termux/files/usr/bin/remove
mv $HOME/FmCodex/files/dx-X-Fm.sh /data/data/com.termux/.FmCodex/
chmod +x  /data/data/com.termux/.FmCodex/
...
donotchange() {
    ...
    VERSION="$D1/dx.txt"
    INPUT_FILE="$HOME/FmCodex/files/.zshrc"
    THEME_INPUT="$HOME/FmCodex/files/.FmCodex.zsh-theme"
    OUTPUT_ZSHRC="$HOME/.zshrc"
    OUTPUT_THEME="$HOME/.oh-my-zsh/themes/FmCodex.zsh-theme"
    TEMP_FILE="$HOME/temp.zshrc"  # Actual temporary file

    # Use sed to replace X-Fm√ with the name and save to temporary file
